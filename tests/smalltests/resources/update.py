...
def get_definition_string(resources, prefix, name, enum_name, list_name):
  ...
  for resource in resources:
    ...
    if res_name in pairs:
      print "Duplicate resource name found: %s" % res_name
      sys.exit(1)
...
def get_animation_definition_string(animation_files, anim_prefix, img_prefix,
  ...
  for anim_file in animation_files:
    ...
    if anim_name_bases[image_name] in used_anim_names:
      print "Duplicate animation name found: %s" % anim_name_bases[image_name]
      sys.exit(1)

    used_anim_names.append(anim_name_bases[image_name])
...
music = walk_directory(MUSIC_SUBDIR, MUSIC_EXTS)
...
music_write = get_definition_string(music, MUSIC_PREFIX, 'Music',
                                    MUSIC_ENUM, MUSIC_PATHVAR)
include_file.write(image_write)
include_file.write(animation_write)
include_file.write(sound_write)
include_file.write(music_write)
include_file.write('\n#endif\n')
